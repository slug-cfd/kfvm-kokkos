...
subList = [(Sx**2 + Sy**2 + Sz**2,S**2),(D + tau + p,A),(A**2 - S**2,B**2),(sqrt(1 - S**2/A**2),Ws)]

print('======= Helper terms ===========')

print('d(Winv)dD')
#print(latex(simplify(diff(1/W,D).subs(subList))))
#pprint(simplify(diff(1/W,D).subs(subList)))

print('d(Winv)dSx')
#print(latex(simplify(diff(1/W,Sx).subs(subList))))
#pprint(simplify(diff(1/W,Sx).subs(subList)))

print('d(Winv)dT')
#print(latex(simplify(diff(1/W,tau).subs(subList))))
#pprint(simplify(diff(1/W,tau).subs(subList)))
...
#pprint(collect(factor(simplify(dhWdSx).subs(subList)),A))

print('d(hW)dT')
#print(latex(collect(factor(simplify(dhWdT).subs(subList)),A)))
#pprint(collect(factor(simplify(dhWdT).subs(subList)),A))

print('======= Pressure derivatives ===')
print('dpdD')
#pprint(collect(simplify(dPdD.subs(subList)),A))
print(latex(collect(simplify(dPdD.subs(subList)),A)))

print('dpdSx')
#pprint(collect(simplify(dPdSx.subs(subList)),A))
print(latex(collect(simplify(dPdSx.subs(subList)),A)))

print('dpdT')
#pprint(collect(simplify(dPdT.subs(subList)),A))
print(latex(collect(simplify(dPdT.subs(subList)),A)))
