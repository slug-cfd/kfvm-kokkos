...
namespace KFVM {
  ...
  namespace Stencil {
    ...
    VectorValuedRA::VectorValuedRA(double eps_,
      ...
      M(K - N - 1),
      epsCont(K/2,0),
      ipiv(K/2,std::vector<int>(stenSize,0)),
      C(K/2,{stenSize,stenSize})
    {
	...
	for (int j=0; j<stenSize; j++) {
	  ...
	  A(i,j) = SE::K<KFVM_D_DECL(SE::EvalFunctional::Average,
	                             SE::EvalFunctional::Average,
	                             SE::EvalFunctional::Average)>(ep,KFVM_D_DECL(dx,dy,dz));
	}
      }
    }
