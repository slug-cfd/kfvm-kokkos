...
namespace KFVM {
  ...
  namespace Stencil {
    ...
    ReconVectors::ReconVectors(double eps_,
                               int monoDeg_,
	...
	for (int j=0; j<stenSize; j++) {
	  double KFVM_D_DECL(dx = xs[i] - xs[j],
			     dy = ys[i] - ys[j],
			     dz = zs[i] - zs[j]);
	  C(i,j) = SE::K<KFVM_D_DECL(EvalFunctional::Average,
            EvalFunctional::Average,
	                             EvalFunctional::Average)>(eps,
	...
	for (int j=0; j<monos.nMono; j++) {
	  C(i,j + stenSize) = Monomials::mono<KFVM_D_DECL(EvalFunctional::Average,
            EvalFunctional::Average,
            EvalFunctional::Average)>(monos.deg[j],KFVM_D_DECL(xs[i],ys[i],zs[i]));
    ...
    int ReconVectors::c_dgetrs(ReconVectors::Matrix<double>& A,
                               std::vector<int>& ipiv,
                               std::vector<double>& b)
    {
      char trans = 'N';
